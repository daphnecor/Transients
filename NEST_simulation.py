...
'''
NEWEST VERSION 28/02/20
'''

class UpDownPatterns:
    
    
    def __init__(self, N_total=1200, stim_start=50., stim_end=200., simtime=1000., n_threads=8):
       
        self.NE = int((N_total/5)*4)
        self.NI = int((N_total/5)*1)
        ...
        self.J_ex = 6.
        self.J_in = -95.
        self.eps = 0.1  # connection probability
    
        self.resolution = 0.1  # temporal resolution of simulation in ms. Kumar2008: 0.1
        self.delay = 1.5  # synaptic delay in the network
    
        self.n_threads = n_threads 
        self.stim_start = stim_start # start applying current
        self.stim_end = stim_end # end applying current
        self.simtime = simtime # simulation time
    
        self.set_stim_amps()  # input current corresponding to 0/1 in pattern
        
        
    def set_stim_amps(self, sub_fr=0.9, sup_fr=1.01):
        ...
        self.stim_amps = np.array([sub_fr, sup_fr]) * I_th
